...
warming_slices = settings.warming_slices
...
def index():
	...
	session['warming_lvl_avail']=list(warming_lvl_dict['en'].keys())
	session['warming_lvl']='2.0'
	session['warming_lvl_ref']='ref'
	print(session['warming_lvl_avail'])
	print(session['warming_lvl'])
	...
	session["season_avail"]     = list(settings.seasons.keys())
	...
	session["region_avail"] = [list(list(COU._region_names.keys()))[list(list(COU._region_names.values())).index(name)] for name in sorted(list(COU._region_names.values()))]
...
def choices():
	...
	if True:
		...
		sorted_regions = [s['country']] + [reg for reg in [list(COU._region_names.keys())[list(COU._region_names.values()).index(name)] for name in sorted(list(COU._region_names.values()))] if reg in s['region_avail']]
		...
		s['warming_lvl_avail']=[list(warming_lvl_dict[lang].keys())[list(warming_lvl_dict[lang].values()).index(name)] for name in sorted(list(warming_lvl_dict[lang].values()))]
		...
		s['season_avail']= [s['season']] + [sea for sea in season_dict.keys() if sea != s['season']]
		form_season.seasons.choices = zip(s['season_avail'],[season_dict[sea]['name'][lang] for sea in s['season_avail']])
		...
		if s['season']!='year' and lang=='en':season_add_on=' in '+season_dict[s['season']]['name'][lang]
		if s['season']!='year' and lang=='fr':season_add_on=' en '+season_dict[s['season']]['name'][lang]
...
def indicator_choice():
	...
	#index=session['indicator_avail'].index(session['indicator'])
	#session['indicator_avail'][index],session['indicator_avail'][0]=session['indicator_avail'][0],session['indicator_avail'][index]
	if ind_dict[session['indicator']]['time_step']=='yearly':    session["season_avail"]=['annual']
	if ind_dict[session['indicator']]['time_step']=='monthly':    session["season_avail"]=list(settings.seasons.keys())
	if session["season"] not in session["season_avail"]: session["season"] = 'annual'
	return redirect(url_for('choices'))
...
def country_choice():
	...
	session["region_avail"]     = [list(COU._region_names.keys())[list(COU._region_names.values()).index(name)] for name in sorted(list(COU._region_names.values()))]
	session['region']     = session["region_avail"][0]
