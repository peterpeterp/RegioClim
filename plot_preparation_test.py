...
periods={'ref':[1986,2006],'2030-2050':[2030,2050]}


for country in ['SEN']:
  for scenario in ['rcp45']:
    for proj_period in ['2030-2050']:
      for indicator in ['tas']:
        ...
        for region in COU[country]._masks['360x720_lat_89.75_-89.75_lon_-179.75_179.75']['lat_weighted'].keys():
          transient_plot='static/images/'+country+'/'+indicator+'_'+region+'_transient__.png'
          if os.path.isfile(transient_plot+'23423')==False:
            fig,ax=plt.subplots(nrows=1,ncols=1,figsize=(5,4))
            print ewembi.average['lat_weighted'].keys()
            COU[country].plot_transients(ewembi,region=region,running_mean_years=20,ax=ax,title='',ylabel=None,label='observation',color='green')
            COU[country].plot_transients(cordex,region=region,running_mean_years=20,ax=ax,title='',ylabel=None,label='projection',color='red')
          ...
          if cordex.time_format=='monthly':
            annual_cycle_plot='static/images/'+country+'/'+indicator+'_'+region+'_annual_cycle_'+proj_period+'-'+ref_period_name+'__.png'
            if os.path.isfile(annual_cycle_plot+'23423')==False:
              fig,ax=plt.subplots(nrows=1,ncols=1,figsize=(5,4))
              COU[country].plot_annual_cycle(ewembi,period=periods['ref'],mask_style='lat_weighted',region=region,ax=ax,title='',ylabel=None,label='observation',color='green')
              COU[country].plot_annual_cycle(cordex,period=periods[proj_period],mask_style='lat_weighted',region=region,ax=ax,title='',ylabel=None,label='projection',color='red')
